...
foreach ($f in $files) {
    ...
    $replacement = @'
        ...
        const celestialBodies = [
            { name: "Sun", r: 35, orbit: 0, speed: 0, color: '#FFD700', angle: 0 },
            { name: "Mercury", r: 4, orbit: 60, speed: 0.02, color: '#A8A8A8', angle: Math.random() * Math.PI * 2 },
            { name: "Venus", r: 8, orbit: 95, speed: 0.015, color: '#E3BB76', angle: Math.random() * Math.PI * 2 },
            { 
                name: "Earth", r: 10, orbit: 145, speed: 0.01, color: '#4fc3f7', angle: Math.random() * Math.PI * 2,
                children: [
                    { name: "Moon", r: 3, orbit: 22, speed: 0.05, color: '#D3D3D3', angle: 0,
                        children: [
                            { name: "Pragyan Rover", r: 1.5, orbit: 6, speed: 0.1, color: '#ffeb3b', angle: 0 }
                        ]
                    },
            ...
            { 
                name: "Mars", r: 7, orbit: 200, speed: 0.008, color: '#ff5252', angle: Math.random() * Math.PI * 2,
                children: [
                    { name: "ISRO MOM", r: 2, orbit: 12, speed: 0.06, color: '#E1AD01', angle: 0 }
                ]
            },
            { name: "Jupiter", r: 22, orbit: 280, speed: 0.005, color: '#D39C7E', angle: Math.random() * Math.PI * 2 },
            { name: "Saturn", r: 18, orbit: 370, speed: 0.003, color: '#EAD6B8', angle: Math.random() * Math.PI * 2, hasRings: true },
            { name: "Uranus", r: 13, orbit: 450, speed: 0.002, color: '#4B70DD', angle: Math.random() * Math.PI * 2 },
            { name: "Neptune", r: 12, orbit: 520, speed: 0.001, color: '#274687', angle: Math.random() * Math.PI * 2 }
        ];
        ...
        function drawBody(body, cx, cy, ctx) {
            ...
            ctx.save();
            ctx.shadowBlur = body.name === 'Sun' ? 50 : 15;
